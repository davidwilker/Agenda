...
type
  TformClienteConsulta = class(TForm)
    ...
    procedure dbgPesquisaClientesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
...
begin
  ...
  begin
    ...
    try
      ...
      formClienteCadastro.editNascimentoCliente.Text  := dbgPesquisaClientes.DataSource.DataSet.FieldByName('DT_NASCIMENTO').AsString;
      formClienteCadastro.editRg.Text                 := dbgPesquisaClientes.DataSource.DataSet.FieldByName('NR_RG').AsString;
      formClienteCadastro.editCepCliente.Text         := dbgPesquisaClientes.DataSource.DataSet.FieldByName('NR_CEP').AsString;
      formClienteCadastro.editEnderecoCliente.Text    := dbgPesquisaClientes.DataSource.DataSet.FieldByName('DS_ENDERECO').AsString;
...
procedure TformClienteConsulta.dbgPesquisaClientesKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if ( Key = VK_DELETE )AND ( NOT ( dbgPesquisaClientes.DataSource.DataSet.IsEmpty ) ) then
  begin

    Cliente.prcDeleta(dbgPesquisaClientes.DataSource.DataSet.FieldByName('ID_CLIENTE').AsInteger);

    Cliente.fncConsulta(editPesquisa.Text);
